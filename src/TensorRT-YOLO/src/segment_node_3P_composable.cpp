...
#include "rclcpp_components/register_node_macro.hpp" // AÑADIDO
...
{
public:
    YoloBatchNode(const rclcpp::NodeOptions &options)
        : Node("yolo_batch_node", options),
          enable_inferred_video_writing_(false),
          enable_mask_video_writing_(false),
          video_fps_(10.0)
    {
        // --- 1. Declarar TODOS los parámetros ---
        this->declare_parameter<std::string>("image_topic", "/camera_front/image_raw");
        this->declare_parameter<std::string>("output_topic_suffix", "default");
        this->declare_parameter<bool>("enable_inferred_video", false);
        this->declare_parameter<bool>("enable_mask_video", false);
        this->declare_parameter<std::string>("video_path", "/home/david/ros_videos/segment_node_3P_out");
        this->declare_parameter<double>("video_fps", 10.0);
        this->declare_parameter<int>("video_width", 1920);
        this->declare_parameter<int>("video_height", 1200);
        this->declare_parameter<std::string>("engine_path", "yolo.engine"); // Parámetro que faltaba
        ...
        // --- 2. Obtener TODOS los parámetros y asignarlos a las variables miembro ---
        image_topic_name_ = this->get_parameter("image_topic").get_value<std::string>();
        output_topic_suffix_ = this->get_parameter("output_topic_suffix").get_value<std::string>();
        enable_inferred_video_writing_ = this->get_parameter("enable_inferred_video").get_value<bool>();
        enable_mask_video_writing_ = this->get_parameter("enable_mask_video").get_value<bool>();
        video_output_path_str_ = this->get_parameter("video_path").get_value<std::string>();
        video_fps_ = this->get_parameter("video_fps").get_value<double>();
        single_video_frame_size_ = cv::Size(
            this->get_parameter("video_width").get_value<int>(),
            this->get_parameter("video_height").get_value<int>()
        );
        ...
        RCLCPP_INFO(this->get_logger(), "Subscribing to: [%s] with transport [%s]", image_topic_name_.c_str(), image_transport_type_.c_str());
        ...
        //     RCLCPP_ERROR(this->get_logger(), "Failed to create CUDA stream: %s", cudaGetErrorString(status));
        // }
        ...
        try
        {
            ...
            if (!model_)
            {
                throw std::runtime_error("Failed to load the engine");
            }
...
RCLCPP_COMPONENTS_REGISTER_NODE(YoloBatchNode)
