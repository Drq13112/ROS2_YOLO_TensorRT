...
/**
 * @brief Convierte un array de NumPy a un objeto deploy::Image.
 *
 * Esta función maneja arrays 2D (Alto, Ancho) para escala de grises y
 * arrays 3D (Alto, Ancho, Canales) para imágenes a color o escala de grises.
 *
 * @param pyarray Un array de NumPy con formato HWC o HW y tipo de dato uint8.
 * @return deploy::Image Un objeto de imagen listo para la inferencia.
 * @throws std::invalid_argument si el formato del array no es soportado.
 */
deploy::Image PyArray2Image(py::array& pyarray) {
    ...
    if (buf_info.format != py::format_descriptor<uint8_t>::format()) {
        throw std::invalid_argument("Se esperaba un array con tipo de dato uint8.");
    }

    int height, width, channels;

    if (pyarray.ndim() == 3) { // Formato HWC (Alto, Ancho, Canales)
        height   = buf_info.shape[0];
        width    = buf_info.shape[1];
        channels = buf_info.shape[2];
    } else if (pyarray.ndim() == 2) { // Formato HW (Alto, Ancho) para escala de grises
        height   = buf_info.shape[0];
        width    = buf_info.shape[1];
        channels = 1; // La imagen en escala de grises tiene 1 canal.
    } else {
        throw std::invalid_argument("El array de entrada debe ser 2D (HW) o 3D (HWC). Se recibió un rango de: " + std::to_string(pyarray.ndim()));
    }

    void* data = buf_info.ptr;
    return deploy::Image(data, width, height, channels);
}
