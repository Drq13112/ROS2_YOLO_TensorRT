...
def generate_launch_description():
    ...
    reverse_x_srv =  ExecuteProcess(
        cmd=[[
            FindExecutable(name='ros2'),
            " service call ",
            "/camera_front/pylon_ros2_camera_node/set_reverse_x ",
            "std_srvs/srv/SetBool",
            ' "{data: True}"',
        ]],
        shell=True
    )
    reverse_y_srv =  ExecuteProcess(
        cmd=[[
            FindExecutable(name='ros2'),
            " service call ",
            "/camera_front/pylon_ros2_camera_node/set_reverse_y ",
            "std_srvs/srv/SetBool",
            ' "{data: True}"',
        ]],
        shell=True
    )
    reverse_set_trigger_source_srv =  ExecuteProcess(
        cmd=[[
            FindExecutable(name='ros2'),
            " service call ",
            "/camera_front/pylon_ros2_camera_node/set_trigger_source ",
    ...
    register_events = RegisterEventHandler(
                OnProcessStart(
                    target_action=pylon_ros2_camera_node,
                    on_start=[
                        LogInfo(msg="Pylon Camera Node started, calling services"),
                        reverse_x_srv,
                        reverse_y_srv,
                        reverse_set_trigger_source_srv,
                    ],
                )
            )
